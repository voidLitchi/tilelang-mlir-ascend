...
static Value createStageSubview(OpBuilder &builder, Location loc, Value source,
                                MemRefType sourceType, Value stageIndex,
  ...
  LLVM_DEBUG({
    DBGS() << "  [CreateStageSubview] Expanded Source Rank: "
           << expandedSourceRank << "\n";
    DBGS() << "  [CreateStageSubview] Original Source Rank: "
           << originalSourceRank << "\n";
    DBGS() << "  [CreateStageSubview] Input Offsets Count: "
           << originalOffsets.size() << "\n";
    debugPrintOFRArray("  [CreateStageSubview] Input Original Offsets",
                       originalOffsets);
  });

  SmallVector<OpFoldResult> normalizedOffsets, normalizedSizes,
      normalizedStrides;
  ...
  if (missingDims < 0) {
    llvm::errs() << "ERROR: More offsets (" << originalOffsets.size()
                 << ") than rank (" << originalSourceRank << ")\n";
    assert(false && "Invalid subview params");
  }

  if (missingDims > 0) {
    LLVM_DEBUG(DBGS() << "  [CreateStageSubview] Detected " << missingDims
                      << " implicit dims. Padding...\n");
  ...
  if ((int)normalizedOffsets.size() != originalSourceRank) {
    llvm::errs() << "FATAL: Normalization failed. Size: "
                 << normalizedOffsets.size()
  ...
  LLVM_DEBUG({
    DBGS() << "  [CreateStageSubview] Final Counts: Offsets="
           << newOffsets.size() << ", Expected=" << expandedSourceRank << "\n";
    if ((int)newOffsets.size() != expandedSourceRank) {
      DBGS() << "  [CreateStageSubview] CRITICAL MISMATCH DETECTED BEFORE "
                "CREATE!\n";
    }
  });

  if ((int)newOffsets.size() != expandedSourceRank) {
    llvm::errs() << "FATAL: Mismatch before create. Got " << newOffsets.size()
                 << " expected " << expandedSourceRank << "\n";
    assert(false && "Rank mismatch");
  }

  auto subviewOp = builder.create<memref::SubViewOp>(loc, source, newOffsets,
                                                     newSizes, newStrides);
  ...
  return builder.create<memref::CollapseShapeOp>(loc, subviewOp.getResult(),
                                                 reassociation);
}
...
public:
  static bool expand(Operation *op) {
    ...
    for (auto allocOp : targets) {
      ...
      // 保存 multi_buffer 属性，防止后续 pass 找不到
      auto multiBufferAttr =
          markOpToRemove->getAttrOfType<IntegerAttr>("hivm.multi_buffer");
      // 移除旧的 MarkOp
      ...
      // 直接用新 alloc 的结果替换所有旧 workspace 的使用
      workspaceValue.replaceAllUsesWith(newAlloc.getResult());
      allocOp.erase();

      // 新建 MarkOp 保留 multi_buffer 标签（MarkOp 本身不产生新值）
      builder.setInsertionPointAfter(newAlloc);
      auto newMarkOp = builder.create<annotation::MarkOp>(allocOp.getLoc(),
                                                          newAlloc.getResult());
      if (multiBufferAttr)
        newMarkOp->setAttr("hivm.multi_buffer", multiBufferAttr);
...
public:
  ScopeToForConverter(Operation *scopeOp, ArrayRef<Value> workspaceValues,
                      int32_t numStage, Value outerIV)
      : scopeOp_(scopeOp), workspaceValues_(workspaceValues),
        numStage_(numStage), outerIV_(outerIV) {}
  ...
  bool convert() {
    ...
    Value cNumStage = builder.create<arith::ConstantOp>(
        loc, builder.getI32Type(), builder.getI32IntegerAttr(numStage_));
    Value c1 = builder.create<arith::ConstantOp>(loc, builder.getI32Type(),
                                                 builder.getI32IntegerAttr(1));

    auto newFor =
        builder.create<scf::ForOp>(loc, c0, cNumStage, c1, ValueRange{});
    ...
    Value constNumStage = builder.create<arith::ConstantOp>(
        loc, builder.getI32Type(), builder.getI32IntegerAttr(numStage_));
    ...
    Value addIdx = builder.create<arith::IndexCastOp>(
        loc, builder.getIndexType(), addI32); // index 类型
    ...
    // 替换新循环体内所有对 outerIV_ 的使用为
    // addIdx，但跳过我们刚刚创建的操作本身
...
private:
      ...
      if (auto constOp = lhs.getDefiningOp<arith::ConstantOp>()) {
        if (auto factor = constOp.getValue().dyn_cast<IntegerAttr>()) {
          if (auto subFactor = getIndexFactor(rhs, baseI32)) {
            llvm::errs() << "  [getIndexFactor] Found mul: " << factor.getInt()
                         << " * " << *subFactor << " = "
                         << (factor.getInt() * *subFactor) << "\n";
  ...
  void handleOuterCmpOps(scf::ForOp innerFor, Block *innerBody, Value addI32,
    ...
    // 设置插入点为 addI32 定义之后
    builder.setInsertionPoint(innerBody,
                              ++addI32.getDefiningOp()->getIterator());
    ...
    for (auto cmpOp : cmpsToClone) {
      ...
      for (Value operand : cmpOp.getOperands()) {
        ...
        if (factor.has_value()) {
          if (*factor == 1) {
            ...
            Value newI32 =
                builder.create<arith::MulIOp>(loc, addI32, constFactor);
            if (operand.getType().isIndex()) {
              newOperands.push_back(builder.create<arith::IndexCastOp>(
      ...
      auto newCmp = builder.create<arith::CmpIOp>(
          loc, cmpOp.getPredicate(), newOperands[0], newOperands[1]);
      cmpOp.getResult().replaceUsesWithIf(
          newCmp.getResult(), [&](OpOperand &use) {
            return use.getOwner()->getBlock() == innerBody;
          });
    }
  }

  void adjustOperationsInLoop(scf::ForOp forOp, Value addI32, Value addIdx,
                              int32_t numStage) {
    ...
    if (inductionVar.getType() != builder.getIndexType()) {
      indexIv = builder.create<arith::IndexCastOp>(loc, builder.getIndexType(),
                                                   inductionVar);
    }
    ...
    for (auto copyOp : copiesToAdjust) {
      adjustCopyOp(copyOp, indexIv, builder,
                   copyOp.getSource() == workspaceValues_[0] /* simplified */,
                   copyOp.getTarget() == workspaceValues_[0], addI32, numStage);
    }
  }

  void adjustWorkspaceSubviewOp(memref::SubViewOp subview, Value indexIv,
                                OpBuilder &builder, Value addI32,
                                int32_t numStage) {
    ...
    for (size_t i = 0; i < origOffsets.size(); ++i) {
      ...
      if (auto val = ofr.dyn_cast<Value>()) {
        ...
        if (factor.has_value()) {
          llvm::errs() << "  [adjustWorkspaceSubviewOp] Found factor "
          ...
          llvm::errs()
              << "  [adjustWorkspaceSubviewOp] Replaced with new index: ";
          newIdx.print(llvm::errs());
          llvm::errs() << "\n";
          continue;
        } else {
          llvm::errs() << "  [adjustWorkspaceSubviewOp] No factor found, "
                          "keeping original\n";
        }
      }
    ...
    LLVM_DEBUG(DBGS() << "  Calling createStageSubview with Rank "
                      << currentSourceType.getRank() << "\n");

    Value newResult =
        createStageSubview(builder, loc, source, currentSourceType, indexIv,
  ...
  void adjustCopyOp(memref::CopyOp copyOp, Value indexIv, OpBuilder &builder,
                    bool fixSource, bool fixTarget, Value addI32,
                    int32_t numStage) {
    ...
    auto subviewOp =
        builder.create<memref::SubViewOp>(loc, ws, offsets, sizes, strides);
...
public:
  PipelineLoopProcessor(scf::ForOp pipelineLoop,
  ...
  bool process() {
    ...
    LLVM_DEBUG(DBGS() << "Processing pipeline loop with tilelangir.num_stages="
                      << numStage << ", workspace count="
                      << workspaceValues_.size() << "\n");
    ...
    Value constNumStage = builder.create<arith::ConstantOp>(
        loc, builder.getI32Type(), builder.getI32IntegerAttr(numStage));
    Value newUpper =
        builder.create<arith::DivSIOp>(loc, oldUpper, constNumStage);
...
struct TileLangIREnableMultiBuffer
    : public impl::TileLangIREnableMultiBufferBase<
          TileLangIREnableMultiBuffer> {
  using Base =
      impl::TileLangIREnableMultiBufferBase<TileLangIREnableMultiBuffer>;
  using Base::Base;
...
void TileLangIREnableMultiBuffer::runOnOperation() {
    ...
    func.walk([&](scf::ForOp forOp) {
      if (forOp->getAttr("tilelangir.num_stages")) {
        if (!forOp->getAttr("tilelangir.pipeline_alg")) {
          StringAttr algAttr = StringAttr::get(forOp->getContext(), "unroll");
          forOp->setAttr("tilelangir.pipeline_alg", algAttr);
        }
  ...
  if (g_subviewProcessedCount != 6) {
    llvm::errs() << "WARNING: Expected 6 subviews to be processed, but got "
                 << g_subviewProcessedCount << ".\n";
    llvm::errs() << "This implies some subviews were skipped or the pass "
                    "crashed early.\n";
  }
}
